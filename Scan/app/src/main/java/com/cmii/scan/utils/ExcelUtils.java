...
public class ExcelUtils {
     ...
     * @param colName
     */
    public static void initExcel(String fileName, String[] colName) throws IOException, WriteException {
        format();
        WritableWorkbook workbook = null;
        try {
            File file = new File(fileName);
            System.out.println("文件存储目录" + file.getPath() + String.valueOf(file.getParentFile().exists()));
            if (!file.exists()) {
                file.createNewFile();
            }
            workbook = Workbook.createWorkbook(file);
            WritableSheet sheet = workbook.createSheet("电子发票", 0);
            //创建标题栏
...
//              sheet.mergeCells()
                ...
                e.printStackTrace();
                Toast.makeText(c, "导出Excel失败", Toast.LENGTH_SHORT).show();
            } finally {
                if (writebook != null) {
                    try {
                        writebook.close();
